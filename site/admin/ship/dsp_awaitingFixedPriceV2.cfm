...
<!--- get the items which are fixed price --->
		<cfquery name="sqlTemp" datasource="#request.dsn#" >
				...
				e1.byrEmail,
				u.itemqty,
				ebitems.galleryurl as galleryurl2,
				u.use_pictures
				...
				left JOIN ebitems ON e2.itmItemID = ebitems.ebayitem
...
<cfif isdefined("sqlTemp")>
		<cfoutput>
			...
			<cfoutput query="sqlTemp" maxrows="#_paging.RowsOnPage#" startrow="#_paging.StartRow#">
				...
				<cfquery datasource="#request.dsn#" name="get_dataitems" >
					select internal_itemSKU2
					from items
					where item = '#sqlTemp.itmSKU#'
		...
		--->
					...
					<!---<cfif varDontDisplay>itemHide rowHidden</cfif>--->"
					 onmouseover="this.className='rowHighlight'"<cfif CurrentRow Mod 2>onmouseout="this.className='rowOdd'"<cfelse>onmouseout="this.className='rowEven'"</cfif> >
					<td align=center>
					...
					<td align=center>#get_dataitems.internal_itemSKU2#</td>
