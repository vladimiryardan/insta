<cfquery datasource="#request.dsn#" name="sqlDefault">
	SELECT item, title, description, weight, weight_oz, bold, border, highlight, startprice_real, startprice, 
	buy_it_now, internal_itemCondition, age, itemManual, itemComplete, itemTested, retailPackingIncluded, specialNotes, 
	internalShipToLocations, upc,
	mpnNum, mpnbrand, sub_description
	,width ,height ,depth, isbn
...
<cfset DynamicDescription =  #Replace(DynamicDescription, '{specialNotes}', sqlDefault.specialNotes, 'ALL')#>
	...
	<cfquery datasource="#request.dsn#">
		...
		(	ebayaccount, itemid, scheduledBy, Title, Description, 
			PackedWeight, PackedWeight_oz,  StartingPrice, ReservePrice, BuyItNowPrice, 
			Bold, Border, Highlight, PackageSize,upc)
		VALUES
		(
			22,
			...
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#Left(sqlDefault.title, 80)#">,
			...
			,<cfqueryparam cfsqltype="cf_sql_varchar" value="#sqlDefault.upc#">
		)
	</cfquery>
	<cfquery name="sqlAuction" datasource="#request.dsn#">
		SELECT a.*
