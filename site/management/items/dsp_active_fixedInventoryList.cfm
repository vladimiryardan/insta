			...
			<cfquery name="getFixedInventory" datasource="#request.dsn#">
				...
				<cfloop index="i" list="#attributes.FieldNames#" DELIMITERS="," >
					...
					<cfif LEFT(i, 5) is "ITEM_" >						
						...
						<cfif isdefined("attributes.receivedNamazon") >
							...
							<cfset LogAction("#itmID# - Changed to amazon Inventoryitem")>						
						<cfelseif isdefined("attributes.btnItemReceived")>
							<!---#itmID#<br>--->
							<cfquery datasource="#request.dsn#">
								update items
								set status = <cfqueryparam cfsqltype="cf_sql_integer" value="3">
								where item = <cfqueryparam cfsqltype="cf_sql_varchar" value="#itmID#">
							</cfquery>		
							<cfset LogAction("#itmID# - Changed to item received")>						
						</cfif>
...
<h3>Matched SKU</h3>
	...
	<cfif getInternalSKu.RecordCount gte 1>
			<cfquery name="get_InstaItemCount" datasource="#request.dsn#">
						SELECT items.item
						  ...
						  (
						   ...
						   and (
						   	items.internal_itemCondition != 'amazon' and 
						   	items.internal_itemCondition != 'AS-IS' 
						  	<!--- and  items.internal_itemCondition != 'New with Defect' 20161014. Patrick says remove--->
						   )
			...
			<cfif get_InstaItemCount.RecordCount gte 1>
				<form action="index.cfm?dsp=management.items.active_fixedInventoryList&item=#attributes.item#" method="post">
				...
				<tr><td>
					...
					<div style="text-align:center">
						...
						<input type="submit" name="btnItemReceived" value="Change Selected to Item recieved">
						<br><br><input type="submit" name="receivedNamazon" value="Change All checked to Amazon Item condition & Status: Item Received">
					</div>
